        ...
        <div>
          {orders
            .filter(
              (order) =>
                activeTab === "Semua" ||
                (activeTab !== "Belum Bayar" && order.status === activeTab) // Mengecualikan status "Belum Bayar"
            )
            .slice(0, activeTab === "Semua" ? 1 : undefined) // Hanya tampilkan satu produk di tab "Semua"
            .map((order, index) => (
              <div
                key={index}
              ...
              >
                ...
                <div className="mt-4 md:mt-0 text-right">
                  ...
                  {/* Buttons */}
                  {order.status === "Belum Bayar" && (
                    <button className="mt-2 px-6 py-2 bg-black text-white text-sm font-bold rounded-lg">
                      BAYAR SEKARANG
                    </button>
                  )}
